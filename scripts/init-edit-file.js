...
function copyIgnoreFile(destPath) {
  const gitIgnorePath = path.resolve(`${global.sourcePath}/src/common/gitignore.txt`)
  fs.copyFileSync(gitIgnorePath, path.resolve(destPath + '/.gitignore'))
}

function editFileWithTemplate(destPath) {
  let depFile = fs.readFileSync(path.resolve(`${global.sourcePath}/src/hf-vue-${global.projectType}/dependencies.json`), 'utf8')
  ...
  let pgFile = fs.readFileSync(path.resolve(`${global.sourcePath}/src/hf-vue-${global.projectType}/package.json`), 'utf8')
  pgFile = pgFile.replace('{{projectName}}', global.projectName)
  fs.writeFileSync(path.resolve(destPath + '/package.json'), pgFile, 'utf8')

  let scssVarFile = fs.readFileSync(path.resolve(`${global.sourcePath}/src/hf-vue-${global.projectType}/variables.scss`), 'utf8')
